/**
 * Google Maps API Integration for custom platform services
 * 
...
function getMapsConfig(): MapsConfig {
  const baseUrl = ENV.mapsProxyUrl || "https://maps.googleapis.com";
  const apiKey = ENV.mapsApiKey;

  if (!baseUrl || !apiKey) {
    throw new Error(
      "Google Maps credentials missing: set GOOGLE_MAPS_API_KEY (and MAPS_PROXY_URL if using a proxy)"
    );
  }
