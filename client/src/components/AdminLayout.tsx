...
export default function AdminLayout({ children, title }: AdminLayoutProps) {
  ...
  const isNonAdmin = Boolean(user && user.role !== "admin");

  useEffect(() => {
    if (!isNonAdmin) return;
    setLocation("/dashboard");
  }, [isNonAdmin, setLocation]);

  const navItems = [
    { path: "/admin", label: t.nav.dashboard, icon: Home },
    { path: "/admin/trainees", label: t.nav.trainees, icon: Users },
    { path: "/admin/trainers", label: t.nav.trainers, icon: UserCheck },
    { path: "/admin/subscriptions", label: t.nav.subscriptions, icon: Calendar },
    { path: "/admin/payments", label: t.nav.payments, icon: CreditCard },
    { path: "/admin/workouts", label: t.nav.workoutPlans, icon: Dumbbell },
    { path: "/admin/exercises", label: t.nav.exerciseLibrary, icon: Library },
    { path: "/admin/attendance", label: t.nav.attendance, icon: Activity },
    { path: "/admin/marketing", label: t.nav.marketing, icon: MessageSquare },
    { path: "/admin/nutrition", label: t.nav.nutrition, icon: Salad },
    { path: "/admin/supplements", label: t.nav.supplementStore, icon: ShoppingBag },
    { path: "/admin/notifications", label: t.nav.myNotifications, icon: Bell },
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <p className="text-muted-foreground">{t.nav.adminPanel}</p>
          <Button asChild>
  ...
  if (isNonAdmin) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center space-y-4">
          <div className="text-2xl font-bold text-foreground">Redirecting...</div>
          <p className="text-muted-foreground">Your account is not admin. Taking you to dashboard.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="p-6 border-b border-border">
        <Link href="/admin" className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-muted-foreground">{t.nav.adminPanel}</div>
          </div>
        </Link>
      </div>
